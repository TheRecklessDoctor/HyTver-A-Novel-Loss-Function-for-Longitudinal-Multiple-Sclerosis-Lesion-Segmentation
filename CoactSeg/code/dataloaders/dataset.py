...
class MS(Dataset):
    """ MS Dataset """
    def __init__(self, base_dir:str|None = None, split:str='train', num=None, transform:transforms.Compose|None=None, cda=True):
    ...
    def __getitem__(self, idx:int):
        ...
        # sample['image_1'], sample['image_2'], sample['label'] = sample['image_1'].squeeze(0), sample['image_2'].squeeze(0), sample['label'].squeeze(0)
        return sample
...
class Crop_3D(object):

    def __init__(self, output_size:int, mode):
        self.crop_size = output_size
        self.mode = mode
    
    def __call__(self, sample):
        tensor_img_1, tensor_img_2, tensor_lab = sample['image_1'], sample['image_2'], sample['label']

        assert self.mode in ['random', 'center'], "Invalid Mode, should be \'random\' or \'center\'"

        crop_size = [self.crop_size] * 3

        D, H, W = tensor_img_1.shape
        ...
        cropped_img_1 = tensor_img_1[rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
        cropped_img_2 = tensor_img_2[rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]
        cropped_lab = tensor_lab[rand_z:rand_z+crop_size[0], rand_y:rand_y+crop_size[1], rand_x:rand_x+crop_size[2]]

        return {'image_1': cropped_img_1.contiguous(), 'image_2': cropped_img_2.contiguous(), 'label': cropped_lab.contiguous()}


def crop_3d(tensor_img_1, tensor_img_2, tensor_lab, crop_size, mode):
    assert mode in ['random', 'center'], "Invalid Mode, should be \'random\' or \'center\'"
    if isinstance(crop_size, int):
        crop_size = [crop_size] * 3

    _, _, D, H, W = tensor_img_1.shape
...
class BatchSampler(Sampler):
    "Iterate through only one set of indices i.e the primary ones as the data"
    "for the secondary ones are not available."
    def __init__(self, primary_indices: list[int], primary_batch_size: int):
        self.primary_indices = primary_indices
        self.primary_batch_size = primary_batch_size
...
def iterate_once(iterable: list[int]) -> npt.NDArray[int]:
    return np.random.permutation(iterable)


def iterate_eternally(indices: list[int]) -> Iterable[Any]:
...
def grouper(iterable:Iterable[int], n:int) -> Iterable[Any]:
