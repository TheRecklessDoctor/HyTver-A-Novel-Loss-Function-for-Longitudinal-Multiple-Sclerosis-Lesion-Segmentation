...
def var_all_case(model, num_classes, logger_path, loss_name, iter, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, act:str="relu"):
    ...
    for image_path in loader:
        if "MSSEG2" in image_path:
            ...
            image_2 = h5f['image_2'][:]
            label = h5f['label'][:]
            assert image_1.shape == image_2.shape, "sizes not the same: {}".format(image_path)
            dice = 0
    ...
    if (iter == 1) or (iter % 5 == 0):
        with open(logger_path + "{}_{}_performance.csv".format(loss_name, act), 'a') as f:
            write = csv.writer(f)
            write.writerow([iter, avg_dice])
            # f.writelines('iter:{}: {}\n'.format(iter, avg_dice))
    return avg_dice

def test_all_case(patch_size, loss_logger_path, loss_func, model_name, num_outputs, model, image_list, num_classes, stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=1, nms=0, act:str="relu"):
    ...
    for image_path in loader:
        if "MSSEG2" in image_path:
            ...
            prediction_1, prediction_2, prediction_sub, _, _, _ = test_single_case_all(model, image_1, image_2, stride_xy, stride_z, patch_size, num_classes=num_classes)
            ...
            if np.sum(prediction)==0:
                ...
                single_metric = calculate_metric_percase(prediction, label[:], count=count, loss_logger_path = loss_logger_path, loss_func = loss_func, act=act)
            ...
            if np.sum(prediction)==0:
                ...
                single_metric = calculate_metric_percase(prediction, label[:], printVals=True)
            total_metric2 += np.asarray(single_metric)

        if save_result:
            nib.save(nib.Nifti1Image(prediction_1.astype(np.float32), np.eye(4)), test_save_path +  "%s_%02d_pred_1.nii.gz" % (loss_func, ith))
            nib.save(nib.Nifti1Image(prediction_2.astype(np.float32), np.eye(4)), test_save_path +  "%s_%02d_pred_2.nii.gz"  % (loss_func, ith))
            nib.save(nib.Nifti1Image(prediction_sub.astype(np.float32), np.eye(4)), test_save_path +  "%s_%02d_pred_sub.nii.gz"  % (loss_func, ith))
            nib.save(nib.Nifti1Image(image_1[:].astype(np.float32), np.eye(4)), test_save_path +  "%s_%02d_img_1.nii.gz"  % (loss_func, ith))
            nib.save(nib.Nifti1Image(image_2[:].astype(np.float32), np.eye(4)), test_save_path +  "%s_%02d_img_2.nii.gz"  % (loss_func, ith))
            nib.save(nib.Nifti1Image((image_2-image_1).astype(np.float32), np.eye(4)), test_save_path +  "%s_%02d_img_sub.nii.gz"  % (loss_func, ith))
            nib.save(nib.Nifti1Image(label[:].astype(np.float32), np.eye(4)), test_save_path +  "%s_%02d_gt.nii.gz"  % (loss_func, ith))
    ...
    with open(test_save_path+'../{}_{}_performance.txt'.format(model_name, act), 'w') as f:
...
def test_single_case_all(model, image_1, image_2, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                # test_patch_b = torch.cat([test_sub, test_sub, test_sub], dim=1)
                
                with torch.no_grad():
                    y1, y2, y3 = model(test_patch)
                    # print(f"y3 contains NaN: {torch.isnan(y3).any()}, Inf: {torch.isinf(y3).any()}")
                    # print(f"y3 min: {y3.min()}, y3 max: {y3.max()}, y3 mean: {y3.mean()}")
                    # y3 = torch.clamp(y3, min=-10, max=10)
                    y1, y2, y3 = F.softmax(y1, dim=1), F.softmax(y2, dim=1), F.softmax(y3, dim=1)
                    # y1, y2, y3  = torch.sigmoid(y1), torch.sigmoid(y2), torch.sigmoid(y3)
                ...
                # print("cnt min:", cnt.min(), "max:", cnt.max())
                # print("cnt updated:", cnt[xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]].sum())
    ...
    label_map_1 = (score_map_1[0]>0.5).astype(int)
    label_map_2 = (score_map_2[0]>0.5).astype(int)
    label_map_sub = (score_map_sub[0]>0.5).astype(int)
    # print(label_map_sub)
    if add_pad:
...
def calculate_metric_percase(pred, gt, count:int=0, loss_logger_path:str = None, loss_func:str = None, act:str="relu"):
    ...
    print("predicted label shape: ", N)
    print("ground truth shape: ", M)
    ...
    with open(loss_logger_path + "individual_performance.txt", 'a') as f:
        if count == 0:
            f.writelines('{}:{}\n'.format(loss_func, act))
        f.writelines("  TPg:{}, M:{}, TPa:{}, N:{}\n".format(TPg, M-1, TPa, N-1))
        f.writelines("  sel:{}, pl:{}, f1:{}\n".format(sel, pl, F1))
        f.writelines("  dice:{}, jc:{}, 95hd:{}, asd:{}, pr:{}, se:{}, sp:{}\n".format(dice, jc, hd, asd, precision, se, sp))
